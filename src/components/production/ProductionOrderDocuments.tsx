...
import type { ProductionOrderDocumentRow } from '../../types/database';

// ---------------------------------------------------------------------------
// Props
// ---------------------------------------------------------------------------

export interface ProductionOrderDocumentsProps {
...
export function ProductionOrderDocuments({ productionOrderId }: ProductionOrderDocumentsProps) {
  const { toast } = useToast();
  const fileInputRef = useRef<HTMLInputElement>(null);

  const [docs, setDocs] = useState<ProductionOrderDocumentRow[]>([]);
  ...
  const fetchDocs = useCallback(async () => {
    ...
    setDocs((data as ProductionOrderDocumentRow[]) ?? []);
    setLoading(false);
  }, [productionOrderId]);
  ...
  async function handleFiles(files: FileList | File[]) {
    ...
    if (validFiles.length === 0) {
      toast({ title: 'Invalid file type', description: 'Only PDF files are allowed.', variant: 'error' });
      return;
    }
    ...
    for (const file of validFiles) {
      ...
      const storagePath = `production-order-docs/${productionOrderId}/${safeName}`;
    ...
    setUploading(false);
    if (uploaded > 0) {
      toast({ title: 'Upload complete', description: `${uploaded} PDF(s) uploaded.`, variant: 'success' });
      await fetchDocs();
    } else {
  ...
  async function handleDownload(doc: ProductionOrderDocumentRow) {
  ...
  async function handleDelete(doc: ProductionOrderDocumentRow) {
    await supabase.storage.from('media-files').remove([doc.storage_path]);
    await supabase.from('production_order_documents').delete().eq('id', doc.id);
        ...
        {uploading ? (
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-sm text-primary-500">
              Drop PDFs here or <span className="font-medium text-accent">browse</span>
            </p>
            <p className="text-xs text-primary-400">PDF · max 50 MB</p>
          </div>
        )}
      </div>
    </section>
